...
public class TFC_M1 extends TFCAttribute {
	...
	public short getExternalDefinition() { 
		return FUELCOM_EQUATION.SELECTION_TFC_M1;
	};
		
	@Override
	public double TFC(SpreadParmsAttribute sa, int flag, double CFB, double SFC, OutVariable<Double> CFC) {
		CFC.value = sa.cfl() * CFB * sa.pc();			// equation 66 ( with mods for m1, m2 )
	...
	public boolean equals(Object other) {
		if (other == null)
			return false;
		return (other instanceof TFC_M1);
	}
	...
	public ca.wise.fuel.proto.TfcAttribute serialize(SerializeProtoOptions options) {
		...
		TfcM1.Builder msgRet = TfcM1.newBuilder();
		msgRet.setVersion((int)serialVersionUID);

		TfcM1.Parms.Builder msg = TfcM1.Parms.newBuilder();
	...
	@Override
	public TFC_M1 deserialize(Message proto) {
		TfcM1 msgRead;
		...
		if(msgRead.hasParms()) {
			TfcM1.Parms msg = msgRead.getParms();
