...
public class DBH_4 extends DBH_HuangCommon {
	...
	public DBH_4(final DBH_4 other) {
		super(other);
	}
	
	public Object clone() throws CloneNotSupportedException {
		DBH_4 other = (DBH_4)super.clone();
		return other;
	}
	...
	public double dbh(double height) {
		...
		if (h <= 0.0)
			return 0.0;
		double dbh = m_b1 + m_b2 * Math.pow(h, m_b3);
		return dbh;
	}
	...
	public ca.wise.fuel.proto.DbhAttribute serialize(SerializeProtoOptions options) {
		...
		Dbh4.Builder msgRet = Dbh4.newBuilder();
		...
		if(!serialized) {
			Dbh4.Parms.Builder msg = Dbh4.Parms.newBuilder();
		...
		builder.setFour(msgRet);
		return builder.build();
	}

	@Override
	public DBH_4 deserialize(Message proto) {
		Dbh4 msgRead;
		...
		if (msgRead.hasParms()) {
			Dbh4.Parms msg = msgRead.getParms();
