...
public class DBH_1 extends DBH_HuangCommon {
	...
	public DBH_1(final DBH_1 other) {
		super(other);
	}
	
	@Override
	public Object clone() throws CloneNotSupportedException {
		DBH_1 other = (DBH_1)super.clone();
		return other;
	}
	...
	public double dbh(double height) {
		...
		if (h <= 0.0)
			return 0.0;
		double dbh = m_b1 * Math.pow(h, m_b2) * Math.exp((0.0 - m_b3) * h);
		return dbh;
	}
	...
	public ca.wise.fuel.proto.DbhAttribute serialize(SerializeProtoOptions options) {
		...
		Dbh1.Builder msgRet = Dbh1.newBuilder();
		...
		if(!serialized) {
			Dbh1.Parms.Builder msg = Dbh1.Parms.newBuilder();
		...
		builder.setOne(msgRet);
		return builder.build();
	}

	@Override
	public DBH_1 deserialize(Message proto) {
		Dbh1 msgRead;
		...
		if (msgRead.hasParms()) {
			Dbh1.Parms msg = msgRead.getParms();
